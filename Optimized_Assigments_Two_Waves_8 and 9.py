# Compares the impact of greedy and optimized assignments on infection spread under two waves of infection
# Experiment corresponding to figures 8 and 9
    ...
    if diff != []:
        ...
        for i in range(len(testing)):
            if infInfo[i] == 'I':
                ...
                rate[i] = rate[i]*gam
    else: avgdiff=0 # Here for learning charting
...
for i in range(Grid_X*Grid_Y): 
    if i%7<=1: # PR
        for j in range(Grid_X*Grid_Y):
            ...
            if i%7<=1: A[i][j] = random.uniform(0,1) # ->PR
            if i%7>1 and i>16: A[i][j] = random.uniform(0,0.1) # ->HW
    if i%7>1 and i<14: # WR
        for j in range(Grid_X*Grid_Y):
            if i==16: A[i][j] = random.uniform(0.05,0.2) # ->TR
...
while T <= duration:
    ...
    if T%8==0:
        ...
        CPRegTrack[Tp], CPOptTrack[Tp] = np.mean([CP_zone_[g] for g in Grid.keys()]), np.mean([CP_zone_O[g] for g in Grid.keys()]) #CP_zone_[zoneOfChoice], CP_zone_O[zoneOfChoice]
        for i in range(N):
            if S[i]=='I':
                InfRegTrack[Tp]+=1 
            if So[i]=='I':
