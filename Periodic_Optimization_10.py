# Compares the impact of greedy and optimized assignments on infection spread under two waves of infection with periodic updates
# Experiment corresponding to figures 10
...
def makeA(mu, st, Grid_X, Grid_Y):
    # Form transition matrix
    values = np.zeros(8)
    for i in range(8): values[i] = np.random.normal(mu,st)
    values = np.sort(values)
    A=np.zeros((Grid_X*Grid_Y,Grid_X*Grid_Y))
    for i in range(Grid_X*Grid_Y): 
        if i%7<=1: # PR
            for j in range(Grid_X*Grid_Y):
                if i==16: A[i][j] = random.uniform(0,0.1) # ->TR
                if i%7>1 and i<14: A[i][j] = random.uniform(0,0.1) # ->WR
                if i%7<=1: A[i][j] = values[1] # ->PR
                if i%7>1 and i>16: A[i][j] = random.uniform(0,0.1) # ->HW
        if i%7>1 and i<14: # WR
            for j in range(Grid_X*Grid_Y):
                if i==16: A[i][j] = values[6] # ->TR
                if i%7>1 and i<14: A[i][j] = values[5] # ->WR
                if i%7<=1: A[i][j] = random.uniform(0,0.1) # ->PR
        ...
        if i%7>1 and i>16: # HW
            for j in range(Grid_X*Grid_Y):
                ...
                if i%7>1 and i<14: A[i][j] = random.uniform(0,0.1) # ->WR
                if i%7<=1: A[i][j] = random.uniform(0,0.1) # ->PR
                if i%7>1 and i>16: A[i][j] = values[7] # ->HW
        if i==16: # TR
            for j in range(Grid_X*Grid_Y):
                if i==16: A[i][j] = values[0] # ->TR
                if i%7>1 and i<14: A[i][j] = values[3] # ->WR
                if i%7<=1: A[i][j] = values[2] # ->PR
                if i%7>1 and i>16: A[i][j] = values[4] # ->HW

    for i in range(Grid_X*Grid_Y): A[i] = (1/np.sum(A[i]))*A[i]
    return A
...
# Transition matrix and page rank
A=makeA(2807.5, 1250.350530851249, Grid_X, Grid_Y)
rank = getPR(A)
...
# Plot alpha learning
plt.plot([i*3 for i in range(len(avgAlpha))], avgAlpha, color='red', label='Observed Mean Alpha Greedy')
plt.plot([i*3 for i in range(len(avgAlpha))], [alphReal for i in range(len(avgAlpha))], color='red', linestyle='dashed', label='True Mean Alpha Greedy')
plt.legend
plt.xlabel('Time')
plt.ylabel('Alpha')
plt.title('Learned Alpha over Time')
plt.tight_layout()
plt.show()

# Plot beta learning
plt.plot([i*3 for i in range(len(avgBeta))], avgBeta, color='red', label='Observed Mean Beta Greedy')
plt.plot([i*3 for i in range(len(avgBeta))], [betaReal for i in range(len(avgBeta))], color='red', linestyle='dashed', label='True Mean Beta Greedy')
plt.legend
plt.xlabel('Time')
plt.ylabel('Beta')
plt.title('Learned Beta over Time')
plt.tight_layout()
plt.show()
