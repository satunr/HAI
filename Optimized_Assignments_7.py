# Compares the impact of greedy and optimized assignments on infection spread
# Experiment corresponding to figure 7
...
duration, T = 60, 0 
R0 = 3.2
sigma, gamma, delta = 0.25, 0.1, 0.025
beta = gamma * R0
r = 1.8288
...
holddiff = []
CPRegTrack, CPOptTrack, InfRegTrack, InfOptTrack = np.zeros(duration+1), np.zeros(duration+1), np.zeros(duration+1), np.zeros(duration+1)
...
# Graph
# ====================
plt.plot([i for i in range(len(CPRegTrack))], CPRegTrack, label='Greedy Uniform', color='blue')
plt.plot([i for i in range(len(CPOptTrack))], CPOptTrack, label='Optimization', color='red')
plt.legend()
plt.xlabel('Time')
...
plt.plot([i for i in range(len(InfRegTrack))], InfRegTrack, label='Greedy Uniform', color='blue')
plt.plot([i for i in range(len(InfOptTrack))], InfOptTrack, label='Optimization', color='red')
plt.legend()
plt.xlabel('Time')
