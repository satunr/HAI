# Model true and predicted CP and infectivity based on accurate mobility data between grids
# Experiment corresponding to figure 5a
...
def seird(L, S, beta, prob, gamma, alpha_decay, CPs, delta, gridLoc, CP_zone_, dist=1.8288):
    # Update CP and infectivity of individuals based on new locations
    global T, tested_infected, LogX, LogY, Inf, zoneOfChoice
    ...
    else: CP_zone_ = {g: np.mean([CPs1[person] for person in CPs1.keys() if gridLocNew[person] == g]) for g in Grid.keys()}
    print (CP_zone_)
    print (CP_zone_pred_, '\n')
...
CP_zone_ = None
while T <= duration:
    print('Time: ', T, np.mean(list(CPs.values())))
    S, CPs, L, gridLoc, CP_zone_ = seird(L, S, beta, prob, gamma, alpha_decay, CPs, delta, gridLoc, CP_zone_)
